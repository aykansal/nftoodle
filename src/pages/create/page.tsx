...
import type { NFTCache } from "@/lib/types";
  ...
  const fetchPageData = useCallback(async (page: number) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/api/nfts?page=${page}&limit=${ITEMS_PER_PAGE}`
      );
          ...
          >
            {currentItems.map((nft, index) => (
              <Link
                ...
                to={`/create/${index}?imageUrl=${encodeURIComponent(nft)}`}
              >
                <motion.div
