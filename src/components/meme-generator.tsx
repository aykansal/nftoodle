...
export function MemeGenerator({ defaultImage }: MemeGeneratorProps) {
  ...
  useEffect(() => {
    ...
    ctx.font = `bold ${fontSize}px ${
      SQUID_ELEMENTS.fonts[selectedFont as keyof typeof SQUID_ELEMENTS.fonts] ||
      "Impact"
    }`;
  ...
  const handleSave = async () => {
    ...
    formData.append("originalImage", defaultImage); // Optional, ensure it's a string or convert to Blob

    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/memes/create`,
        formData,
        {
          headers: {
                ...
                <div className="relative w-full aspect-[16/9] max-w-[1600px] mx-auto">
                  ...
                  <canvas
                    ...
                    className={`w-full h-full object-contain rounded-lg shadow-xl ${
                      isPlacingEmoji ? "cursor-crosshair" : "cursor-default"
                    }`}
                  />
                </div>
                      ...
                      {Object.entries(SQUID_ELEMENTS.emojis).map(
                        ([key, emoji]) => (
                          <button
                            ...
                            className={`squid-button text-2xl aspect-square ${
                              isPlacingEmoji && selectedEmoji === emoji
                                ? "ring-2 ring-[#FF0B7A]"
                                : ""
                            }`}
                          >
